...
import { Search, Filter, CheckCircle, Plus, AlertTriangle, X } from 'lucide-react';
...
export function AlertsEscalations() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (payload.id) {
        // payload.description = `[ID: ${payload.id}] ${payload.description}`; // Don't prepend ID
        (payload as any).customId = payload.id; // Send as customId to backend
        delete payload.id; // Still remove ID if it's meant to be auto-generated by DB
      }
  ...
  const filteredAlerts = alerts.filter(alert => {
    ...
    const matchesSearch =
      (alert.customId || alert.id).toLowerCase().includes(searchQuery.toLowerCase()) ||
                    ...
                    return (
                      <TableRow key={alert.id}>
                        <TableCell className="font-mono text-xs">
                          {alert.customId || alert.id.substring(0, 8)}
                        </TableCell>
                        <TableCell>
