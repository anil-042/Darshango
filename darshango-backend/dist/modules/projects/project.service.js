...
const createProject = (projectData) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const dbProject = {
        ...
        location_lat: (_a = projectData.location) === null || _a === void 0 ? void 0 : _a.lat,
        location_lng: (_b = projectData.location) === null || _b === void 0 ? void 0 : _b.lng,
...
const getAllProjects = (...args_1) => __awaiter(void 0, [...args_1], void 0, function* (filters = {}) {
    ...
    if (filters.agencyId)
        query = query.eq('implementing_agency_id', filters.agencyId);
...
const updateProject = (id, updateData) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (updateData.component)
        dbUpdate.component = updateData.component;
    if (updateData.implementingAgencyId)
        dbUpdate.implementing_agency_id = updateData.implementingAgencyId;
    if (updateData.executingAgencyId)
        dbUpdate.executing_agency_id = updateData.executingAgencyId;
    if (updateData.state)
...
const recalculateProjectStats = (projectId) => __awaiter(void 0, void 0, void 0, function* () {
    // Milestones
    const { data: milestones } = yield supabase_1.supabase
        .from('milestones')
        .select('status')
        .eq('project_id', projectId);
    const totalMilestones = (milestones === null || milestones === void 0 ? void 0 : milestones.length) || 0;
    const completedMilestones = (milestones === null || milestones === void 0 ? void 0 : milestones.filter(m => m.status === 'Completed').length) || 0;
    const progress = totalMilestones > 0 ? Math.round((completedMilestones / totalMilestones) * 100) : 0;
    // Funds
    const { data: funds } = yield supabase_1.supabase
        .from('funds')
        .select('type, amount, uc_status')
        .eq('project_id', projectId);
    const released = (funds === null || funds === void 0 ? void 0 : funds.filter(f => f.type === 'Release' || f.type === 'Agency Release' || f.type === 'District Allocation').reduce((acc, f) => acc + (f.amount || 0), 0)) || 0;
    const utilized = (funds === null || funds === void 0 ? void 0 : funds.filter(f => f.type === 'Utilization').reduce((acc, f) => acc + (f.amount || 0), 0)) || 0;
    const pendingUCs = (funds === null || funds === void 0 ? void 0 : funds.filter(f => f.uc_status === 'Pending').length) || 0;
    // Inspections
    const { count: inspectionCount } = yield supabase_1.supabase
        .from('inspections')
        .select('*', { count: 'exact', head: true })
        .eq('project_id', projectId);
    // Documents
    const { count: documentCount } = yield supabase_1.supabase
        .from('documents')
        .select('*', { count: 'exact', head: true })
        .eq('project_id', projectId);
